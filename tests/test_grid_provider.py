...
class TestGridProvider(unittest.TestCase):
    ...
    def test_provider_type_error(self):
        provider = GridProvider(int(4), multiplier_fun=1.0)
...
class VisualTests(unittest.TestCase):
    ...
    def test_random_uniform_provider_2d():
        provider = GridProvider(int(2), seed=int(42), multiplier_fun=1.0)
    ...
    def test_random_uniform_provider_3d():
        provider = GridProvider(int(3), seed=int(42), multiplier_fun=1.0)
    ...
    def test_random_chebyshev_density_provider_1d():
        points = GridProvider(dimension=1, seed=int(42), multiplier_fun=1.0)._sample_chebyshev_univariate(num_points=1000)
    ...
    def test_chebyshev_2d_custom_range():
        provider = GridProvider(int(2), seed=int(42), lower_bound=1., upper_bound=4., multiplier_fun=1.0)
        grid = provider.generate(grid_type=GridType.CHEBYSHEV, scale=int(5))
        utils.visualize_point_grid_2d(grid, alpha=1.)
    ...
    def test_chebyshev_3d_custom_range():
        provider = GridProvider(int(3), seed=int(42), lower_bound=1., upper_bound=4., multiplier_fun=1.0)
        grid = provider.generate(grid_type=GridType.CHEBYSHEV, scale=int(5))
        utils.visualize_point_grid_3d(grid, alpha=1.)
...
class TestNumberGridPoints(unittest.TestCase):
    ...
    def test_2d_scale_6(self):
        dim = 2
        scale = 6
        provider = GridProvider(dim, multiplier_fun=1.0)
