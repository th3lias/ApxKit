...
\begin{equation}\label{eq:lagrangeInterpolation}
    \mathcal{I}\left(\FF\right)(x) = \sum_{n=0}^N f(z_n) \ell_n(x)\quad \text{ s.t. }\quad \ell_n(x) \defn \prod_{\substack{j = 0bin \neq j}}^N \frac{x-z_j}{z_n-z_j}
\end{equation}
...
\begin{equation}\label{eq:SmolyakWithDifferences1}
    \cA(q, d) = \sum_{k=d}^q \Delta^{(N)}(\XX^=(k))
\end{equation}
or equivalently
