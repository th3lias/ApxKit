...
from grid.random_grid import RandomGrid
...
class RandomGridProvider(GridProvider):
    ...
    def generate(self, scale: int) -> RandomGrid:
        ...
        if self.rule == RandomGridRule.UNIFORM:
            return RandomGrid(self.input_dim, self.output_dim, scale, self._generate_uniform(n_points),
                              grid_type=self.rule, seed=self.seed)
    ...
    def increase_scale(self, current_grid: Grid, delta: int = 1, keep_old_pts: bool = True) -> Grid:
        ...
        if self.rule == RandomGridRule.UNIFORM:
            if keep_old_pts:
                ...
                return current_grid.vstack(Grid(self.input_dim, self.output_dim, delta, new_pts, grid_type=self.rule))
            else:
                old_scale = current_grid.scale
                del current_grid
                return Grid(self.input_dim, self.output_dim, old_scale + delta,
                            self._generate_uniform(target_no_points), grid_type=self.rule)
        elif self.rule == RandomGridRule.CHEBYSHEV:
            if keep_old_pts:
                ...
                del current_grid
                return Grid(self.input_dim, self.output_dim, old_scale + delta,
                            self._generate_chebyshev(target_no_points), grid_type=self.rule)
